...
class BalancerACTR(nn.Module):
    ...
    def forward(self, loss, fg_mask):
        ...
        Args:
            loss: (B, H, W), Pixel-wise loss
            gt_boxes2d: (B, N, 4), 2D box labels for foreground/background balancing  ## no use
        Returns:
            loss: (1), Total loss after foreground/background balancing
            tb_dict: dict[float], All losses to log in tensorboard
        """
        ...
        # Compute losses
        loss = loss.detach().cpu() * loss.squeeze(1).detach().cpu()*weights
        fg_loss = loss[fg_mask].sum() / num_pixels
        bg_loss = loss[bg_mask].sum() / num_pixels
