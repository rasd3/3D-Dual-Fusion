...
class PointHeadSimple(PointHeadTemplate):
    ...
    def __init__(self, num_class, input_channels, model_cfg, **kwargs):
        ...
        self.cls_layers = self.make_fc_layers(fc_cfg=self.model_cfg.CLS_FC,
    ...
    def assign_targets(self, input_dict):
        ...
        assert gt_boxes.shape.__len__() == 3, 'gt_boxes.shape=%s' % str(
            gt_boxes.shape)
        ...
        extend_gt_boxes = box_utils.enlarge_box3d(
            ...
            extra_width=self.model_cfg.TARGET_CONFIG.GT_EXTRA_WIDTH).view(
                batch_size, -1, gt_boxes.shape[-1])
...
class AuxPointHeadSimple(PointHeadTemplate):
    """
    For auxililary segmentation loss for ACTR
    """
    ...
    def div_batch_dict(self, sconv):
        ...
        def coor2pts(x):
            ...
            ratio = 1600 / sconv.spatial_shape[1]
            pts = (x.to(torch.float) + 0.5) * torch.tensor(
                voxel_size[::-1]).cuda() * ratio
            pts += torch.tensor(pc_range[:3][::-1]).cuda()
            pts = pts[:, [2, 1, 0]]
            return pts

        p_coords = coor2pts(sconv.indices[:, 1:])
        p_coords = torch.cat((sconv.indices[:, 0].unsqueeze(1), p_coords), dim=1)
        batch_dict = {
            'point_features': sconv.features,
            'point_coords': p_coords
        }
        return batch_dict

    def forward(self, sconv, gt_boxes):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        Returns:
            batch_dict:
                ...
                point_part_offset: (N1 + N2 + N3 + ..., 3)
        """
        batch_dict = self.div_batch_dict(sconv)
